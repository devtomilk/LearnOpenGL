...
void render()
{
    ...
    glUniform3f( glGetUniformLocation(cubeShader->program, "spotLight.position"),
		 camera->getPos().x,
		 camera->getPos().y,
		 camera->getPos().z );

    glUniform3f( glGetUniformLocation(cubeShader->program, "spotLight.direction"),
		 camera->getFront().x,
		 camera->getFront().y,
		 camera->getFront().z );
    ...
    {
	model = glm::mat4(1.0f);
	model = glm::translate( model, lampsPositions[i] );
